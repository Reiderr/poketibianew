	...
	if(combat == 128 or combat == 1) and spellNameFromAttacker == -1 then -- ataque basico
	   ...
	   if value == 0 then 
		...
		if critical then -- X-Critical system
		    ...
		    doSendAnimatedText(getThingPos(cid), (value == 0 and "" or value) .. " STK", COLOR_BURN)
		else
		-- 			 doSendAnimatedText(getThingPos(cid), (value == 0 and "" or value), 180)
			doSendAnimatedText(getThingPos(cid), (value == 0 and "" or value), raceCombat.color)
		end
		...
		if(isReflect(cid))then -- reflect system igual GBA
		   if spellName ~= -1 then 
			   if not isInArray({"Team Claw", "Team Slice", "Mega Wing", "Iron Defense"}, spellName) then
				  doSendMagicEffect(getThingPosWithDebug(cid), 135)
				  doSendAnimatedText(getThingPosWithDebug(cid), "REFLECT", 215)
